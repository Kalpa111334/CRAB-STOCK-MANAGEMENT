  ...
  const handleShare = async () => {
    try {
      ...
      if (totalDamagedCrabs > 0) {
        ...
        Object.entries(damageTypeStats).forEach(([damageType, count]) => {
          text += `• ${damageType}: ${count} crabs\n`
        })
        text += `\n`
      }

      // Add dead crab information if any exist
      if (totalDeadCrabs > 0) {
        const totalDeadWeight = deadCrabEntries.reduce((sum, entry) => sum + entry.weight_kg, 0)
        text += `💀 *DEAD CRAB DETAILS*\n`
        text += `🏋️ Total Dead Weight: ${totalDeadWeight} kg\n`
        text += `📊 Average Weight: ${(totalDeadWeight / totalDeadCrabs).toFixed(2)} kg\n\n`
      }

      text += `📊 *BOX STATUS*\n`
      text += `📦 Total: ${totalBoxes}\n`
      text += `✅ Filled: ${filledBoxes}\n`
      text += `❌ Damaged: ${damagedBoxes}\n`
      text += `📭 Empty: ${emptyBoxes}\n\n`
      text += `🔄 *Last Updated*\n`
      text += `⏰ ${new Date().toLocaleTimeString()}\n\n`
      text += `📱 *Crab Stock Guardian System*`
      ...
      if (navigator.share) {
        ...
        toast({
          title: "Status copied!",
          description: "Inventory status has been copied to clipboard.",
          variant: "default"
        })
      }
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Failed to share inventory status.",
        variant: "destructive"
      })
    }
  }
